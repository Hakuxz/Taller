        ...
        if not tipo_raza:
            ...
            while(True):
                ...
                if int(total_puntos) < 12:
                    ...
                    nuevo_personaje = personajes(personaje_dao.buscarID(), nombre_personaje, 'Vivo', 1, int(atributos[3]), int(atributos[4]), int(atributos[5]), 0, int(atributos[0]), int(atributos[1]), int(atributos[2]), int(id_jugador), 0, int(id_raza))
